...
class InitialPath:
    ...
    def omni_model(self, robot_state, vel, sample_time):
        """
        Omnidirectional robot motion model for initial path prediction.
        
        For omnidirectional robots, the control input vel represents:
        vel[0]: velocity in x direction (vx)
        vel[1]: velocity in y direction (vy)
        
        The robot can move in any direction directly without rotation constraints.
        
        Args:
            robot_state: current state [x, y, theta] (3, 1)
            vel: velocity command [vx, vy] (2, 1)
            sample_time: time step
            
        Returns:
            next_state: predicted next state [x, y, theta] (3, 1)
        """
        
        assert robot_state.shape == (3, 1) and vel.shape == (2, 1)

        vx = vel[0, 0]  # velocity in x direction
        vy = vel[1, 0]  # velocity in y direction

        # For omnidirectional robots, movement is direct
        ds = np.array([[vx], [vy], [0]])  # no rotation for now, theta remains constant
