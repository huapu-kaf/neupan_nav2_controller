...
class robot:
    ...
    def linear_omni_model(self, nom_state, nom_u, dt):
        """
        Linear omnidirectional robot model for optimization.
        
        For omnidirectional robots, the kinematics are:
        x_{k+1} = x_k + vx * dt
        y_{k+1} = y_k + vy * dt  
        theta_{k+1} = theta_k + w * dt
        
        Where the control input u = [vx, vy] for omnidirectional velocity.
        For simplicity in the optimization framework, we treat it as:
        - u[0] = resultant velocity magnitude
        - u[1] = angular velocity (similar to diff drive)
        
        But the motion is omnidirectional, so we decompose based on current state.
        
        Args:
            nom_state: nominal state [x, y, theta] (3x1)
            nom_u: nominal control [vx, vy] or [v, w] (2x1)  
            dt: time step
            
        Returns:
            A, B, C: linearized dynamics matrices
        """
        
        # For omnidirectional robots, we use a simplified model:
        # The robot can move in any direction with vx, vy velocities
        # We assume nom_u[0] is vx and nom_u[1] is vy in robot frame
        
        A = torch.Tensor([ [1, 0, 0], [0, 1, 0], [0, 0, 1] ])
        
        # For omnidirectional robot, B matrix allows direct velocity control
        # nom_u[0] = vx (velocity in x direction)
        # nom_u[1] = vy (velocity in y direction)  
        B = torch.Tensor([ [dt, 0], [0, dt], [0, 0] ])
        
        # No nonlinear terms for omnidirectional robot
